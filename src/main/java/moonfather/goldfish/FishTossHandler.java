...
public class FishTossHandler
{
	public static void changeLuck(ItemEntity entityItem, boolean addLuck)
	{
		...
		if (effect == null)
		{
			int level = Math.min(entityItem.getItem().getCount() - 1, Config.MaxLuckLevel.get() - 1); // zero based
			effect = new MobEffectInstance(potionToAdd, Config.LuckEffectDuration.get(), level, true, false);
		...
		else
		{
			...
			level = Math.min(level + entityItem.getItem().getCount(), Config.MaxLuckLevel.get() - 1); // zero based
			time = (time + time + Config.LuckEffectDuration.get()) / 3;
	...
	{
		if (entityItem.level() instanceof ServerLevel world)
		{
			...
			if (entityItem.isInWater())
			{
				...
				while (world.getFluidState(pos).is(FluidTags.WATER))
				{
